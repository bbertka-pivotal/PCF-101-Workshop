...
. Edit the `manifest-pcf.yml` file to specify the Cloud Foundry target the apps are being pushed to, replacing the URL in `CF_TARGET: https://api.cf.yourcompany.com` with the API endpoint for your Cloud Foundry deployment.
